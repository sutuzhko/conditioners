...
export default [
  ...
  },
  {
    ignores: [
      ...
      '**/next-env.d.ts',
      /* Копия контракта маршрутов (issue #883): повторяет то, что Next пишет
         в `.next/types/link.d.ts`, вплоть до `infer _`. Расходиться с
         оригиналом ради линтера нельзя — сверка со сборкой перестанет
         сходиться, а править файл руками запрещено: он генерируется. */
      'apps/web/types/routes-contract.d.ts',
    ],
  },
  {
    rules: {
  ...
  },
  {
     ...
     * (ADR-171), и до пяти — когда своя закрытая отдача появилась у снимка
     * установки в карточке клиента (issue #868): у закрытой отдачи ровно та
     * же природа, что у открытой.
     */
    ...
    files: [
      ...
      'apps/*/src/app/api/admin/clients/*/units/*/photo/route.ts',
    ],
