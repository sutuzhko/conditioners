 ...
 *
 * 🔴 Одно **поле** настроек на проект. Раньше наименование для футера и
 * `legalName` для разметки `Organization` были двумя независимыми полями, и
 ...
 *
 * Строк с появлением `legalShortTitle` стало две, но поле осталось одно, и
 * разойтись данным по-прежнему негде: у общества в футере стоит сокращённое
 * наименование, а в разметку и в политику уходит полное — это официальное имя
 * лица, и сокращать его в документе нельзя. У предпринимателя обе строки
 * совпадают: сокращённого наименования у него не бывает.
 *
