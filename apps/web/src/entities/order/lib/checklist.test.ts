...
const order: ChecklistSource = {
  tools: ['Перфоратор с бурами и удлинителем', 'Стремянка'],
  heightWorks: false,
  payment: 'company',
  price: 38_500,
  units: [unit],
...
describe('сборка чеклиста выезда', () => {
  /**
   * 🔴 Инструмент приезжает из справочника видов работ, а не из таблицы в коде
   * (ADR-343). До переезда список лежал здесь `Record`'ом на три типа наряда —
   * то есть был последним перечнем видов работ в коде: заведи владелец «Чистку
   * дренажа», и монтажник уехал бы на неё без единой строки про инструмент.
   */
  it('инструмент вида работ попадает в чеклист как есть', () => {
    const install = buildChecklist(order);
    const service = buildChecklist({
      ...order,
      tools: ['Мойка высокого давления и пакет для чистки'],
    });
  ...
  /* Вид работ без записанного инструмента — рабочее состояние: владелец
     заводит «Консультацию», и брать на неё нечего. Чеклист от этого не
     перестаёт собираться: позиции, штробление и деньги в нём остаются. */
  it('вид работ без инструмента даёт чеклист без строк инструмента', () => {
    const lines = buildChecklist({ ...order, tools: [] });

    expect(lines).not.toContain('Перфоратор с бурами и удлинителем');
