 ...
 *
 * Роли — учётные записи с логином и паролем (ADR-092): монтажник открывает ту
 * же панель, просто видит из неё меньше разделов. Отдельной сущности
 * «монтажник» рядом с пользователем нет — она дала бы две таблицы людей и
 * вопрос «а если монтажник станет владельцем».
 *
 * 🔴 Порядок значений — лестница доступа сверху вниз (ADR-344), и он тот же,
 * что в перечислении базы: владелец видит всё; администратор — то, что
 * разрешил владелец; менеджер — жизненный цикл клиента; монтажник — свои
 * наряды. Порядок читается сортировкой списка команды, поэтому переставлять
 * значения нельзя, не переставив их в схеме.
 */
export const adminRoleSchema = z.enum(['owner', 'admin', 'manager', 'installer']);
