...
export type DayBlockLike = {
  ...
  /** Разовая: календарная дата — первый день. У повторяемой пусто. */
  readonly day: DayKey | null;
  /**
   * Последний день разовой отлучки включительно: отпуск на две недели — одна
   * запись, а не четырнадцать (ADR-165). Пусто — отлучка на один день, то
   * есть `day`; у повторяемой пусто всегда.
   */
  readonly endDay: DayKey | null;
...
/**
 * Занятость, попадающая на этот день: разовая — попаданием в свой диапазон
 * дат, повторяемая — совпадением дня недели.
 *
 * 🔴 Единственное место, где решается «действует ли отлучка в этот день», —
 * и через него идут все три ответа про занятость: слой календаря, пометка
 * занятых при назначении наряда и предупреждение о пересечении (ADR-165).
 * Разойдись они, монтажник окажется свободен в одном экране и занят в другом.
 *
...
export function blocksOn<Block extends DayBlockLike>(
  ...
  /* 🔴 Диапазон сравнивается строками, а не датами: ключ дня — это
     `2026-08-26`, и у такого формата лексикографический порядок совпадает с
     хронологическим. Разбор в `Date` завёл бы здесь часовой пояс, которого у
     календарной даты нет вовсе (ADR-080). */
  const applied = blocks.filter((block) => {
    if (block.repeat === 'weekly') return block.weekday === weekday;
    if (block.day === null) return false;

    // пустой конец — отлучка на один день: она и есть свой последний день
    return block.day <= day && day <= (block.endDay ?? block.day);
  });
