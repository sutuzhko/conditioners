...
const API_SECTIONS: Readonly<Record<string, PermissionRule>> = {
  /**
   * 🔴 Журнал событий владельческий целиком (ADR-345, ADR-392, ADR-397):
   * читать, кто что сделал, — это читать про себя в том числе, и переключателя
   * на чтение журнала нет. Исключение одно — чистка, и она ниже.
   */
  activity: OWNER_ONLY,
...
const API_OVERRIDES: Readonly<Record<string, PermissionRule>> = {
  ...
  /**
   * 🔴 Чистка журнала — единственное, что администратору в журнале
   * открывается, и открывается своим опасным действием (ADR-345: «чистит
   * владелец; администратор — если владелец разрешил»).
   *
   * Разрешения на раздел рядом нет намеренно: раздела «Журнал» среди
   * тринадцати нет, читать журнал администратору не открывается ничем. Отсюда
   * и странность, которую надо назвать вслух: обладатель «Чистки журнала»
   * чистит то, чего не видит, — экран журнала владельческий. Так решено
   * (ADR-392, ADR-397), и здесь это записано, а не выведено случайно.
   */
  'activity/cleanup POST': needs('activity_purge'),
};
...
const PAGE_SECTIONS: Readonly<Record<string, PermissionRule>> = {
  ...
  profile: ALWAYS,
  /**
   * 🔴 Журнал событий остаётся владельческим целиком (ADR-345, ADR-392):
   * читать, кто что сделал, — это читать про себя в том числе. Экрана у
   * администратора нет ни при каком наборе; единственное, что ему бывает
   * открыто, — чистка, и она живёт ручкой, а не страницей.
   */
  activity: OWNER_ONLY,
};
