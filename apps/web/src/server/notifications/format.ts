...
export function notificationText(payload: NotificationPayload): string {
  if (payload.kind === 'lead') {
    return [
      ...
      '',
      `🧭 Тема: ${payload.topic}`,
      /* 🔴 Подпись отличается от строки контекста «Заказ с карточки»
         намеренно: здесь то, что человек видел в поле формы и подтвердил
         (ADR-129), там — снимок того, что он делал (ADR-133). Одинаковые
         подписи выглядели бы как дубль, и владелец перестал бы читать обе.
         Прочерк, как у соседних полей: пустая строка формы — это факт. */
      `🏷 Модель: ${orDash(payload.model ?? null)}`,
      `👤 Имя: ${payload.name}`,
