...
import { CLIENTS_PATH, type ClientApi } from './model';
...
/**
 * Заведение клиента — окном с собственным адресом (ADR-117).
 *
 ...
 * список клиентов открывают как раз для того, чтобы найти человека. Само окно —
 * из кита (`RouteModal`), здесь только то, что окно заводит.
 *
 * Правка сюда не попадает: она остаётся страницей карточки — это разные по
 * длительности действия.
 */
export function ClientCreateModal({ api }: ClientCreateModalProps) {
  const close = useRouteClose(CLIENTS_PATH);
