...
export const orderManagerContent = {
  ...
  /* Выбор строк и групповое действие (issue #596, #739, макет «Заказы»).
     ...
     🔴 Подписи полосы подобраны так, чтобы её состав не менялся между
     состояниями: полоса стоит над таблицей всегда, и слово, выросшее на
     строку, двигало бы таблицу ровно так же, как это делало появление самой
     полосы (issue #738). Поэтому «Выбрать все на странице», а не «Выбрать все
     наряды на странице»: длинная подпись переносилась на 390 второй строкой. */
  selectAll: 'Выбрать все на странице',
  selectedNone: 'Ничего не выбрано',
  selectedOf: (count: number, total: number): string => `Выбрано ${count} из ${total}`,
  selectionClear: 'Снять выбор',
  selectionClearOff: 'Снимать нечего: ни один наряд не отмечен.',
  bulkGroup: 'Что сделать с выбранными нарядами',
  bulkAssign: 'Назначить',
  bulkAssignLabel: 'Кому назначить выбранные наряды',
  /* Пустое значение поля названо коротко: на 390 поле остаётся 181px, и
     «Выберите монтажника» обрезалось на середине слова. «Кому назначить»
     входит в имя поля целиком — видимая подпись обязана быть частью
     доступного имени (WCAG 2.5.3). */
  bulkPlaceholder: 'Кому назначить',
  ...
  /* Причины отказа кнопки. Названы словами, а не молчанием: отключённая
     кнопка без объяснения — это действие, которого для человека просто нет. */
  bulkOffEmpty: 'Сначала отметьте наряды галочками — назначать пока нечего.',
  bulkOffInstaller: 'Выберите монтажника, которому назначить отмеченные наряды.',
  ...
  historyMargin: 'Маржа',
  /* 🔴 Итог, умолчавший о пропущенных нарядах, владелец прочтёт как полный —
     и решит по нему, какую цену ставить. Поэтому пропуск называется, а не
     замалчивается (ADR-310, issue #628). */
  historyMarginSkipped: (count: number): string =>
    `${count} ${plural(count, 'наряд', 'наряда', 'нарядов')} без закупочной цены`,
  /* Прочерк вместо маржи: материалы списаны, а цены у них нет. Число без них
     завысило бы маржу — соврало бы в ту сторону, ради которой закупочная цена
     и заводилась. */
  /* Подпись работы ниже 1200: рядом стоит сумма, и маржа без слова читалась
     бы как второе число того же ряда. */
  marginCompact: (value: number): string => `маржа ${formatMoney(value)}`,
  marginCompactNone: 'маржа не посчитана',
  marginUnknownHint: (count: number): string =>
    `Маржа не посчитана: у ${count} ${plural(count, 'движения', 'движений', 'движений')} склада нет закупочной цены`,
  ...
  /* 🔴 Вид работ отключили, но у этого наряда он остался (ADR-343). Пункт
     виден выбранным и подписан — иначе поле выглядело бы незаполненным, — но
     выбрать его заново нельзя: отключённый вид новой работы не получает. */
  workTypeOff: (title: string): string => `${title} · отключён`,
  status: 'Статус',
  day: 'Дата',
  time: 'Время',
