...
export const installerContent = {
  ...
  /* 🔴 Без примера с числами. Пример «доп. трасса 1,5 м» стоял здесь дословно
     тем же текстом, который вводят в поле, — и одна и та же строка оказывалась
     на экране трижды: в подсказке, в самом поле и в разборе под ним. Подсказка
     объясняет, что писать словами, а не показывает готовый ответ. */
  extraWorkHint: 'Словами, как скажете клиенту. Метры трассы и короба разберутся сами.',
