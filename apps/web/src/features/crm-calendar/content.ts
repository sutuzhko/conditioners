...
/*
 * 🔴 Перечней видов работ в этом файле нет (ADR-343). Ни у дела, ни у наряда:
 * подпись, значок и краску обе записи привозят из справочника вместе с собой,
 * и «монтаж» в наряде красится той же строкой базы, что «монтаж» в деле —
 * разойтись им больше нечем.
 *
 * Различаются наряд и дело в сетке не цветом (ADR-093): у наряда есть номер,
 * сплошная полоса слева и слово «Наряд» в подписи для скринридера — в
 * монохромном режиме различие остаётся.
 */
...
export const crmContent = {
  ...
  /* 🔴 Слой людей и виды записей прячут записи молча: неделя с делами
     выглядела точно так же, как неделя без них, и владелец шёл заводить
     второе дело поверх уже заведённого (issue #580). Сброс возвращает слой
     целиком, оставляя вид и неделю на месте. */
  agendaNothing: 'На этой неделе записей не видно',
  agendaNothingHint: 'Записи на неделе есть — их скрыли выбранные люди или виды записей.',
  agendaReset: 'Показать все записи',
  ...
  /* 🔴 Вид работ отключили, а у записи он остался (ADR-343). Пункт виден
     выбранным и подписан — иначе поле выглядело бы незаполненным, — но выбрать
     его для новой записи нельзя. */
  kindOff: (title: string): string => `${title} · отключён`,
  fieldDay: 'Дата',
  ...
  /* 🔴 День звучит вместе со временем: перетаскивание вбок меняет именно
     день, и объявление «Перенесено на 10:00» о нём не сказало бы ничего
     (issue #143). */
  movedNote: (date: string, time: string): string => `Перенесено на ${date}, ${time}`,
  ...
  /* 🔴 Объявление называет обе даты: у отпуска, переехавшего на день, важно
     не только начало — «Перенесена на 20 августа» не сказало бы, до какого
     числа человека теперь нет (#144). Однодневная отлучка называет одну. */
  busyMovedNote: (from: string, to: string | null): string =>
    to === null ? `Отлучка перенесена на ${from}` : `Отлучка перенесена: с ${from} по ${to}`,
  busyDrop: 'Снять',
  busyEdit: 'Изменить',
