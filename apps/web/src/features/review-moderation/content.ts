...
export const reviewModerationContent = {
  ...
  /* Заголовок ошибки блока: он называет то, что не приехало, а не раздел
     целиком — вкладки и отбор над списком при этом остаются рабочими. */
  ...
  /* 🔴 Говорит, что произошло и что с этим делать, а не «ошибка загрузки»:
     модератор всё равно решает по отзыву, и знать ему надо именно это. */
  photoGone: 'Фото недоступно',
  photoGoneNote: 'Файл снимка не найден — решайте по тексту отзыва',
