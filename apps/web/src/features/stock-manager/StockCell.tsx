  ...
  return (
    ...
    >
      <button
        ...
        onDrop={onDrop}
      >
        {/* 🔴 Название зоны стоит внутри кнопки, а не подписью ячейки рядом с
            ней. На карточке телефона чип и есть цель, и цель обязана быть
            целой: подпись снаружи оставляла нажимаемыми только цифру — 19,5px
            по ширине при абсолютном минимуме 24 и норме 44 (ADR-183). Голосом
            кнопка называется своим `aria-label`, и он полнее этой подписи. */}
        <span className={styles.zoneTag}>{zoneName}</span>
        {formatQty(qty)}
      </button>
    </td>
  );
