 ...
 * готовыми.
 *
 * 🔴 Залипают обе стороны: слева позиция, справа итог и меню строки (issue
 * #42, #352, #573). Без первой на четвёртой зоне непонятно, чей это остаток;
 * без второй «Итого» — то, ради чего в таблицу и смотрят, — обрывалось краем
 * экрана. По той же причине залипает и меню: действие, до которого нужно
 * доехать через шесть зон, из списка не достаётся. Страница по горизонтали
 * при этом не двигается: прокрутка живёт внутри контейнера. Кнопка
 * «Переместить» остаётся в первой колонке: это самое частое действие раздела,
 * и одно нажатие ему дешевле двух.
 *
            ...
            <tbody>
              {items.map((item, row) => (
                ...
                <tr key={item.id} role="row" data-danger={item.low === true ? '' : undefined}>
                  ...
                  {/* 🔴 Итог стоит предпоследним и залипает с отступом в
                      ширину колонки меню: две колонки, прижатые к нулю,
                      встают друг поверх друга. */}
                  <td
                  ...
                  <td className={styles.menuCell} role="cell">
