...
/**
 * 🔴 Две позиции ниже порога, и вторая — последняя строка списка (issue #749).
 *
 * Пометка последней строки обязана читаться так же, как у строки посреди
 * списка: разделителя под последней строкой нет ни под одной ячейкой. Пока
 * правило кита снимало его только у `td`, заголовок строки оставался с
 * границей, она же перекрашивалась в красный — и владелец видел под названием
 * красный обрубок вместо пометки во всю ширину.
 */
export const ДвеНижеПорога: Story = {
  args: {
    overview: {
      ...overview,
      items: [pipe, bracket, freonLow],
      total: 3,
      itemsTotal: 3,
      lowCount: 2,
      nearCount: 0,
