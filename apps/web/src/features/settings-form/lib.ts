...
/**
 * Сутки в минутах — модуль для приведения времени, а не предел настройки.
 *
 * 🔴 Комментарий здесь раньше называл 1440 «пределом рабочего окна в схеме», и
 * это было неправдой: `scheduleSchema` ограничивает и начало, и конец окна
 * значением 1439, то есть 23:59. Так решено сознательно — календарь прижимает
 * время суток к 1439, и разрешённые 1440 доезжали до панели как «23:59», то
 * есть настройка не сохраняла то, что в неё ввели.
 *
 * Здесь константа нужна только для деления по модулю: 1440 приводится к «00:00»
 * (см. `minutesToTime`), потому что поле времени в браузере суток не знает.
 * Расхождение с 1440 в `entities/crm/lib/load.ts` — не ошибка: там считается
 * длина отрезка на сетке, и запись, идущая до полуночи, обязана доходить до
 * конца дня.
 */
