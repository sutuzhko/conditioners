...
export function ScrollTop() {
  ...
  const toTop = (): void => {
    ...
    /* 🔴 Фокус переезжает в шапку. Кнопка исчезает, как только страница
       ...
       🔴 `preventScroll` обязателен, и без него кнопка не работала вовсе.
       Браузер подтягивает получивший фокус элемент в видимую область — и этим
       отменяет плавную прокрутку, которую мы только что запустили. Замерено на
       стенде: страница уезжала с 11682 до 11309 и вставала, то есть ровно на
       высоту липкой шапки вместо возврата к началу. */
