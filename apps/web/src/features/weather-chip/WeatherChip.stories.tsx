...
/**
 * 🔴 Состояние телефона (issue #254): до 600 чип обязан уложиться в одну
 * строку, поэтому уточнение «ср/сут» и сезонная заметка скрыты. Ширина здесь
 * часть состояния — история задаёт её глобалью, а не обёрткой: медиа-запрос
 * смотрит на окно, а не на контейнер.
 */
export const Phone: Story = {
  name: 'Телефон 375 — одна строка',
  globals: { viewport: { value: 'sm' } },
  args: { weather: { mean: 15, max: 34 } },
};

/** Нижняя граница поддержки: строка обязана остаться целой и без обрезки. */
export const Narrow: Story = {
  name: 'Минимум 320 — без обрезки',
  globals: { viewport: { value: 'xs' } },
  args: { weather: { mean: -15, max: -3 } },
};
