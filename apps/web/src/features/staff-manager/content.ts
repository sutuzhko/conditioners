...
export const staffManagerContent = {
  ...
  /**
   * 🔴 Подпись обещает главное свойство этих переключателей: они действуют
   * немедленно. Человек, снявший раздел, обязан знать, что звонить и просить
   * «перезайти» не нужно, — иначе он будет ждать до завтра и решит, что не
   * сработало.
   */
  accessHint: (name: string): string =>
    `Что открыто в панели: ${name}. Снятый переключатель закрывает раздел сразу — выходить и заходить заново не нужно.`,
