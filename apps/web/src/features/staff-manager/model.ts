...
/**
 * Показатели монтажника в строке списка (issue #602, #629).
 *
 * 🔴 Загрузка считается из нарядов и своего поля в базе не имеет (ADR-310):
 * `durationMin` и `overtimeMin` уже есть у каждого наряда, а рабочее окно
 * задаёт владелец (ADR-138). Две цифры об одном и том же неизбежно разошлись
 * бы, и верить стало бы нечему.
 */
export type StaffRowStats = {
  /** Минуты нарядов текущей недели. */
  readonly loadMin: number;
  /** Норма недели в минутах: рабочее окно × пять рабочих дней. */
  ...
  /**
   * Сколько нарядов за человеком всего, в любом статусе.
   *
   * 🔴 Не то же, что `done`: удаление учётной записи закрыто, пока за
   * монтажником закреплён хоть один наряд — иначе наряд остался бы без
   * исполнителя. По этому числу строка и решает, можно ли удалять.
   */
