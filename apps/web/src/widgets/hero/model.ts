...
/**
 * Короткая подпись чипа для узкой карточки.
 *
 * 🔴 Подписью меняется только надпись, но не значение: в подбор и в заявку
 * уезжает `PlaceType`, а «Дом» — то, что помещается в треть карточки на 320px.
 * Полная подпись остаётся именем кнопки для читалки на всех ширинах, поэтому
 * WCAG «имя содержит видимую подпись» соблюдено: «Дом» входит в «Частный дом».
 */
export const PLACE_SHORT: Readonly<Record<PlaceType, string>> = {
  Квартира: 'Квартира',
  'Частный дом': 'Дом',
  [OFFICE_PLACE_TYPE]: OFFICE_PLACE_TYPE,
};
