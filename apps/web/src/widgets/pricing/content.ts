...
export const pricingText = {
  ...
  totalLabel: 'Монтаж под ключ',
  /* 🔴 Кнопка ведёт к заявке — она и вела туда всегда, врала только подпись
     (issue #555). Выезда на замер в этом бизнесе нет: после заявки и выбора
     модели сразу монтаж, бригада привозит всё с собой. «Вызвать замерщика»
     обещало отдельный визит оценщика и прятало настоящее преимущество —
     монтаж за один выезд, которым торгует соседний блок. */
  apply: 'Оставить заявку',

  /* Полоса итога голосом: пока сумма пересчитывается, читалка обязана
     услышать состояние, а не пустое поле. Скелетон для неё невидим. */
  totalPending: 'Пересчитываем',

  /* 🔴 Третье состояние итога. На верхнем делении шкалы формула перестаёт
     считать честно (entities/price/lib/estimateScope), и вместо числа
     полоса говорит словами: названная сумма обязана совпасть с той, что
     скажут по телефону. */
  /* 🔴 Не «по телефону»: на 320 полоса даёт подписи 174px, а «Считаем по
     телефону» занимает 184 — она переносилась во вторую строку и толкала
     кнопку вниз на 11px, ломая правило `stability`. «По звонку» — 159px,
     уже прежних 163. Длину этой подписи менять только с замером. */
  onSite: 'Считаем по звонку',
  onSiteText:
    'Длинная трасса и мульти-сплит считаются отдельно: сумму назовём по телефону до выезда, а работы сделаем в тот же приезд — бригада везёт всё с собой.',
