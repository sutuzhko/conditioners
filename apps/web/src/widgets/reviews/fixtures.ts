...
/**
 * Отзыв на двенадцать строк. 🔴 Ради него и заведена обрезка (issue #275):
 * в ряду равных карточек такой текст либо вытягивает ряд втрое, либо
 * оставляет соседям полосы пустоты. Показ обрывается по четвёртой строке,
 * содержание остаётся целым.
 */
export const longReviewFixture: ReviewCardData = {
  id: 'demo-long',
  name: 'Виктория',
  rating: 5,
  text:
    'Кондиционер выбирали долго: квартира угловая, окна на юг, и предыдущий ' +
    'мастер уверял, что семёрки хватит. Здесь посчитали площадь вместе с ' +
    'остеклением и объяснили, почему нужна девятка, — без давления, просто ' +
    'показали расчёт. Приехали в назначенное время, вынесли мебель из угла ' +
    'сами, штробили аккуратно и пылесосили сразу за перфоратором. Трассу ' +
    'спрятали в короб, короб подобрали в цвет откосов. Вакуумацию делали при ' +
    'мне, показали манометр до и после. Смета совпала с расчётом по телефону ' +
    'до рубля, доплат за высоту и за длину трассы не выставляли. Через сезон ' +
    'работы кондиционер не шумит и не течёт, фильтры мыть научили. Отдельно ' +
    'спасибо за то, что после монтажа убрали за собой и вынесли упаковку.',
  photo: null,
  avatar: null,
...
/**
 * Шесть отзывов и больше — та граница, с которой лента на десктопе едет сама
 * (`REVIEWS_DRIFT_FROM`). Меньше — ход показывал бы одни заготовки.
 */
export const manyReviewsFixture: readonly ReviewCardData[] = [
  ...reviewsFixture,
  longReviewFixture,
