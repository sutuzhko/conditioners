...
/**
 * Контейнер заготовки, не имеющий собственной геометрии (issue #579).
 *
 * 🔴 Заготовки разделов — фрагменты, а не обёртки, и это записано у каждой из
 * них 🔴-комментарием: страница раскладывает блоки колонкой с общим зазором, и
 * лишний `<div>` съедает зазор ровно на время загрузки — то есть заготовка
 * обещает не ту геометрию, ради которой её и мерили (ADR-239).
 *
 * Но `Skeleton` несёт `aria-hidden`, и фрагмент из одних заготовок не говорит
 * озвучке вообще ничего: раздел молча стоит пустым, пока идут данные. Отсюда
 * `display: contents` — элемент есть в дереве доступности и несёт `aria-busy`,
 * а бокса не создаёт, и дети остаются прямыми детьми той же колонки. Приём в
 * проекте не новый: так же собраны подвал и блок цен.
 *
 * Ставится там, где внутри нет ни одного контейнера с `aria-busy` своего:
 * `RowsSkeleton`, `FieldsSkeleton` и `MonthSkeleton` объявляют его сами, и
 * второй такой же поверх них ничего не добавит.
 */
export function BusyGroup({ children }: { readonly children: ReactNode }) {
  return (
    <div className={styles.busy} aria-busy="true">
      {children}
    </div>
  );
}
