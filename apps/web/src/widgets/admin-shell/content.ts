 ...
 *
 * 🔴 `roles` — это подсказка интерфейса, а не защита. Доступ к чужому разделу
 * закрывают перечень ролей у маршрута (`withRoles`) и проверка роли на самой
 * странице (ADR-092, ADR-344): адреса панели человек знает, он в ней работает.
 */
...
/* ---------- Кому раздел показывается — и кого по его адресу пускает
   ...
   🔴 Перечни приезжают из `entities/staff/access` — того же модуля, по
   которому ставят себе стража страницы и ручки API (ADR-344). Своей копии у
   колонки нет намеренно: список, выписанный здесь второй раз, и есть тот
   способ, которым матрица доступа расходится сама с собой.

   🔴 Значения перечней в бандл колонки Zod не тянут: `access.ts` знает про
   роль только типом. ---------- */
...
export const ADMIN_SECTIONS: readonly AdminSection[] = [
  ...
  /* 🔴 Журнал стоит в конфигурации, а не в колонке: в него заходят с вопросом
     «кто это сделал», а не каждый день. Но стоит он именно здесь, а не нигде —
     раздел без пункта меню открывается только по адресу, набранному наизусть,
     и критерий «владелец находит, что менеджер делал в среду, за три нажатия»
     не выполняется при этом ни при каком числе нажатий (issue #815). */
  {
    href: '/admin/activity',
    title: 'Журнал событий',
    hint: 'Кто и когда что менял в панели',
    icon: 'clock',
    roles: OWNER,
    place: 'settings',
  },
     ...
     дальше к необратимому, «Выйти» последним (ADR-188). */
  {
    ...
    title: 'Настройки',
    hint: 'Компания, цены на монтаж, уведомления и журнал событий',
    icon: 'settings',
    roles: OWNER,
    place: 'bottom',
  },
 ...
 * правилу — как все.
 *
 * 🔴 Адрес вне известных разделов проходит намеренно, и это половина
 * договорённости: раздел не обязан появляться в колонке раньше, чем он готов,
 * — а вторая половина в том, что на самой странице стоит страж (ADR-095).
 * Без стража незнакомый адрес окажется открыт любому вошедшему.
 *
 * Так жил журнал событий: заведённый фазой 1 без пункта в колонке, он
 * закрывался только `requireOwnerPage()`. Пункт он получил фазой 4 (issue
 * #815) — раздел, который ищут по набранному наизусть адресу, не выполняет
 * своего критерия «найти за три нажатия». Сегодня у каждой страницы панели
 * есть и пункт, и страж; мягкость этой строки остаётся для следующего
 * раздела, который заводят по частям.
 */
