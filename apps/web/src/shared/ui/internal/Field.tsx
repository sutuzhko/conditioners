...
/**
 * Ряд отбора: поле, выпадающие списки и действия карточки поиска стоят на
 * одной строке ввода (issue #742).
 *
 * 🔴 Правило живёт в ките, а не в модуле раздела. Подсказка под полем — часть
 * поля, и то, что она делает с соседями по ряду, обязан решать тот, кто её
 * рисует: три раздела равняли ряд по нижнему краю и получали кнопку «Найти»
 * на строке подсказки, четвёртый компенсировал подпись числом, которому после
 * ADR-307 нечего компенсировать.
 *
 * Раскладку ряда — сколько колонок, какой зазор, что переносится первым —
 * по-прежнему задаёт раздел: в очереди обращений одно поле, в статьях поле и
 * три списка. Кит задаёт единственное: чем ряд равняется.
 *
 * 🔴 Класс раздела идёт первым, китовый вторым — как у `tableAboveClassName`:
 * сборщик измерений называет узел первым классом-модулем (ADR-234), и при
 * обратном порядке форма поиска стала бы в диффе безымянным `Field__row`,
 * одинаковым во всех разделах сразу.
 */
export function fieldRowClassName(className?: string | undefined): string {
  return [className, styles.row].filter(Boolean).join(' ');
}

/**
 * Действия ряда отбора — кнопка «Найти» и ссылка сброса рядом с ней.
 *
 * Ставится на тот же контейнер действий, который раздел уже рисует: класс
 * добавляет ровно отступ на строку подписи и ничего не знает про то, сколько
 * в этом контейнере кнопок.
 */
export function fieldRowActionsClassName(className?: string | undefined): string {
  return [className, styles.rowActions].filter(Boolean).join(' ');
}
