...
/**
 * 🔴 Обе величины равны — issue #666.
 *
 * Ноль и ноль владелец видит на первом же входе в панель: до первого
 * закрытого наряда выручки и выплат нет ни у кого. Обе подписи вставали в одну
 * точку и рисовались друг поверх друга. Теперь подпись одна на две линии, и
 * какая где — говорит легенда.
 */
export const EqualEnds: Story = {
  name: 'Равные величины: одна подпись',
  args: {
    series: [
      { id: 'revenue', name: 'Выручка', points: [0, 0, 0, 0, 0, 0] },
      { id: 'payout', name: 'Выплаты', points: [0, 0, 0, 0, 0, 0] },
    ],
    title: 'Выручка и выплаты',
    format: (value: number) => `${value} т₽`,
  },
};

/**
 * 🔴 Величины близкие, но разные — подписи разводятся, а не сливаются в одну.
 *
 * Показать одно число вместо двух здесь нельзя: 214 и 209 — это два разных
 * числа, и одно на их месте было бы неправдой.
 */
export const CloseEnds: Story = {
  name: 'Близкие величины: подписи разведены',
  args: {
    series: [
      { id: 'revenue', name: 'Выручка', points: [120, 180, 150, 240, 210, 214] },
      { id: 'payout', name: 'Выплаты', points: [80, 110, 90, 160, 140, 209] },
    ],
    title: 'Выручка и выплаты',
    format: (value: number) => `${value} т₽`,
  },
...
/**
 * 🔴 Столбцы (issue #589). Отвечают на «сколько было в каждую неделю», а не
 * «как менялось»: у ломаной глаз читает наклон между точками, у столбцов —
 * высоту каждого по отдельности. Легенды нет намеренно — серия одна, и её имя
 * стоит заголовком карточки.
 */
export const Bars: Story = {
  name: 'Столбцы',
  args: {
    kind: 'bars',
    series: [{ id: 'done', name: 'Выполненные наряды', points: [9, 14, 11, 7, 16, 12] }],
    title: 'Заказы по неделям',
  },
};

/** Столбцы за двенадцать недель — та плотность, что стоит в макете «Обзор». */
export const BarsDense: Story = {
  name: 'Столбцы за двенадцать недель',
  args: {
    kind: 'bars',
    series: [
      {
        id: 'done',
        name: 'Выполненные наряды',
        points: [9, 14, 11, 7, 16, 12, 10, 5, 18, 15, 13, 11],
      },
    ],
...
/**
 * Неделя без единого закрытого наряда: столбец нулевой высоты, а не полоска
 * минимального размера. Пририсованный столбик означал бы работу, которой не было.
 */
export const BarsWithZero: Story = {
  name: 'Столбцы с пустой неделей',
  args: {
    kind: 'bars',
    series: [{ id: 'done', name: 'Выполненные наряды', points: [9, 0, 11, 0, 16, 12] }],
    title: 'Заказы по неделям',
  },
};
