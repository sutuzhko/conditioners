...
describe('Отметки шкалы', () => {
  ...
  /* 🔴 Подпись отметки округляется до целого, а сама отметка стоит там, где
     стоит. На шкале 0…1 середина 0,5 подписывалась единицей: на графике
     оказывались две единицы, и линия сетки врала про своё значение. Так
     выглядели «Заказы по неделям», пока недельный максимум был равен
     одному. */
  it('🔴 середина отбрасывается, если её подпись совпала с краем', () => {
    expect(ticksOf({ min: 0, max: 1 })).toEqual([0, 1]);
    expect(ticksOf({ min: 0, max: 2 })).toEqual([0, 1, 2]);
  });
});
