 ...
 *
 * 🔴 `alert` — не направление, и стоит он здесь намеренно (issue #590). Чип у
 * плитки один, место у него одно, и часть показателей отвечает не «выросло или
 * упало», а «пора вмешаться»: у очереди обращений важна не динамика, а то, что
 * самое старое висит вторые сутки. Отдельный проп рядом с `delta` дал бы два
 * взаимоисключающих чипа в одной строке и вопрос «что рисовать, если заданы
 * оба»; здесь ответ следует из типа. Стрелку `alert` не рисует: «↓ 1 сутки»
 * читается ровно наоборот тому, что означает.
 */
export type StatTrend = 'up' | 'down' | 'flat' | 'alert';
...
/* 🔴 Глифы текстовые, все четыре из одной весовой категории. Знак восклицания,
   а не «⚠»: кадры визуальной регрессии снимаются в Linux, а эмодзи-предупреждение
   рисуется разными шрифтами на разных системах и в тёмной теме приезжает
   цветным — то есть чип менял бы вид от машины к машине. */
const GLYPH: Readonly<Record<StatTrend, string>> = { up: '↑', down: '↓', flat: '=', alert: '!' };
