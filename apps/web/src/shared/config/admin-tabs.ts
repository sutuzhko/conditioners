...
export const PANEL_TABS = {
  ...
  /* Модерация отзывов. Пятая вкладка — вопреки макету (ADR-300): он рисовался
     до того, как стало ясно, что архив недостижим. Удалять архивные нельзя
     (инвариант 7), значит их только больше, и «убрать с глаз, но не потерять»
     без своего входа превращается в «убрать и потерять». */
  reviews: ['pending', 'published', 'rejected', 'archived', 'all'],
