...
/**
 * 🔴 Номер из одних нулей арифметически честен: остаток нуля по любому модулю
 * равен нулю, поэтому контрольный разряд у него сходится. Но это не номер, а
 * прочерк, набранный цифрами, — и, пройдя проверку, он напечатался бы в футере
 * и в политике обработки персональных данных как проверенный реквизит.
 */
describe('прочерк, набранный цифрами, не проходит', () => {
  it('одни нули отвергаются у всех номеров', () => {
    expect(isInnCompany('0000000000')).toBe(false);
    expect(isInnPerson('000000000000')).toBe(false);
    expect(isOgrn('0000000000000')).toBe(false);
    expect(isOgrnip('000000000000000')).toBe(false);
  });

  /* Нулевого субъекта РФ не существует — то же правило, что уже стоит у КПП. */
  it('нулевой код региона отвергается у ИНН и КПП', () => {
    expect(isInnCompany('0007083893')).toBe(false);
    expect(isKpp('001701001')).toBe(false);
  });

  it('настоящие номера от этих правил не страдают', () => {
    expect(isInnCompany('7707083893')).toBe(true);
    expect(isOgrn('1027700132195')).toBe(true);
    expect(isInnPerson('710703123450')).toBe(true);
    expect(isOgrnip('314710700012346')).toBe(true);
    expect(isKpp('710701001')).toBe(true);
  });
});
