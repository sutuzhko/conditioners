...
/**
 * Предел длины адреса. Владелец вправе назвать статью длинным заголовком,
 * но адрес из двухсот символов ломается в письмах и мессенджерах.
 */
export const SLUG_MAX_LENGTH = 80;

/**
 * Слаг для адреса страницы: обрезан по длине и никогда не пустой.
 * Заголовок целиком из знаков препинания дал бы пустой адрес — вместо этого
 * страница получает запасное имя и остаётся доступной.
 */
export function pageSlug(source: string, fallback = 'element'): string {
  const trimmed = slugify(source).slice(0, SLUG_MAX_LENGTH).replace(/-+$/g, '');
