...
/**
 * Дробное количество склада: `43,5`, `12 000`, `−1,5`.
 *
 * Три знака после запятой — предел колонки в базе: фреон меряется
 * килограммами, труба метрами, и «полбаллона» — нормальное рабочее состояние.
 *
 * 🔴 Минус приводится к типографскому. Дефис в позиции минуса читается как
 * перенос и рвёт строку ровно там, где число нужно прочитать целиком, — а
 * отрицательный остаток на складе показывается намеренно (ADR-134).
 */
export function formatQuantity(value: number): string {
  return withNbsp(RU_QUANTITY.format(value)).replace(/^[-\u2212]/u, '−');
}
