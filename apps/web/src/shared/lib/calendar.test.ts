...
describe('местное время → момент', () => {
  ...
  it('🔴 день календаря хранится московской полночью, и в UTC это прошлые сутки', () => {
    /* Ловушка, стоившая смены диагностики: отлучка «с 3 сентября» лежит в базе
       как `2026-09-02T21:00Z`, и читалка, печатающая дату по UTC, показывает
       «2026-09-02» — то же, что до переноса на день вперёд. Отсюда вывод
       «сервер ответил 200, а запись не изменилась», хотя изменилась.

       Сдвиг на сутки меняет момент на 24 часа, а дату в UTC — не всегда:
       московская полночь отстоит от неё на три часа. Сверять надо момент
       целиком либо дату, переведённую в пояс работ (`dayKeyOf`). */
    expect(momentOf('2026-09-02', '00:00').toISOString()).toBe('2026-09-01T21:00:00.000Z');
    expect(momentOf('2026-09-03', '00:00').toISOString()).toBe('2026-09-02T21:00:00.000Z');

    // и обратно: тот же момент читается третьим сентября, а не вторым
    expect(dayKeyOf(momentOf('2026-09-03', '00:00'))).toBe('2026-09-03');
  });
...
describe('расстояние между днями', () => {
  it('считает дни через границу месяца', () => {
    // отпуск с 19 августа по 1 сентября — четырнадцать дней, тринадцать шагов
    expect(daysBetween('2026-08-19', '2026-09-01')).toBe(13);
  });

  it('в обратную сторону даёт то же число со знаком минус', () => {
    expect(daysBetween('2026-09-01', '2026-08-19')).toBe(-13);
    expect(daysBetween('2026-08-19', '2026-08-19')).toBe(0);
  });

  it('🔴 не сбивается на переводе часов: у календарной даты пояса нет', () => {
    // в ночь на 29 марта 2026 в Европе сутки короче на час — в UTC этого нет
