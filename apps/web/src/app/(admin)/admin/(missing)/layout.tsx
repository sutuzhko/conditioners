 ...
 *
 * 🔴 Группа заведена ради **кода ответа**, а не ради вида. Когда у группы
 * `(panel)` была заготовка загрузки, она открывала поток ответа раньше, чем
 * страница успевала сказать «не найдено»: статус к тому моменту был
 * отправлен, и `404` подменялся на `200`. Заготовки уехали внутрь страниц
 * (issue #651), но группа остаётся: у несовпавшего адреса нет ни раздела, ни
 * роли, с которой его сверять, — а `(panel)` начинает именно со сверки.
 *
