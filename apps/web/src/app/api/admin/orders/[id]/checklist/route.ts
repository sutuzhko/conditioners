 ...
 *
 * Открыт владельцу и монтажнику (перечень `FIELD`): список сборов ведёт тот,
 * кто собирается. Что монтажник видит только свой наряд, решает репозиторий —
 * там же, где стоит фильтр по исполнителю (CRM.md §6).
 */
 ...
 */
export const PUT = withRoles(FIELD, async (_request, context: Context, session) => {
  const { id } = await context.params;
