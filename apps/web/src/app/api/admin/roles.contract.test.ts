...
const EXPECTED: Readonly<Record<string, RoleListName>> = {
  ...
  /* Журнал событий. Пока ролей в нём одна — владелец: отдельные разрешения
     «Журнал» и «Чистка» приходят вместе с механизмом ADR-344 (issue #823), и
     до него читать историю чужих действий может только он.

     🔴 Метода удаления записи в таблице нет, потому что его нет в дереве
     маршрутов: построчное удаление отсутствует в API как решение, а не как
     недоделка (ADR-345). Появление здесь `activity/[id] DELETE` — повод не
     дописать строку, а спросить, зачем она. */
  'activity GET': 'OWNER',
  'activity/[id] PATCH': 'OWNER',
