...
test.describe('Действия строки списка', () => {
  ...
  test('статья правится и убирается из строки, а отказ ничего не меняет', async ({ page }) => {
    ...
    /* Поиск по доступному имени, а не по тексту подписи. «Заголовок» —
       начало ещё двух подписей формы («Заголовок страницы» на вкладке SEO и
       его двойник в предпросмотре), и нестрогий поиск находил три поля разом.
       Точное совпадение по подписи тоже не годится: у обязательного поля к
       ней дописана звёздочка, и текст подписи — «Заголовок *». В доступное
       имя она не входит: пролёт со звёздочкой помечен `aria-hidden`. */
    const titleField = page.getByRole('textbox', { name: 'Заголовок', exact: true });
