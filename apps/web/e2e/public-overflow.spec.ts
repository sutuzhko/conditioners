...
/**
 * Обещание PRD — оно осталось историей, а не порогом: замер после всех девяти
 * фаз дал 16 410, и сократить остаток вёрсткой нечем (ADR-298, issue #537).
 */
const PRD_PROMISE = 14_000;

/**
 * 🔴 Принятый бюджет. Секции лендинга не режутся: каждая — либо аргумент,
 * который не прочитают, либо текст, который не проиндексируют. Высота на
 * телефоне это прокрутка, а не вес, и настоящая цена — Core Web Vitals, а они
 * в норме с запасом.
 *
 * Порог при этом остаётся порогом: он выше сегодняшнего замера всего на
 * двести пикселей, и правка, добавившая экрану ещё сотню, красит прогон.
 */
const HOME_HEIGHT_BUDGET = 16_600;
...
test(`высота страниц на ${HEIGHT_WIDTH}px`, async ({ page, request }, info) => {
  ...
  expect(
    ...
    `высота главной на ${HEIGHT_WIDTH}px: обещание PRD было ${PRD_PROMISE},` +
      ` принятый бюджет ${HOME_HEIGHT_BUDGET} (ADR-298)`,
  ).toBeLessThanOrEqual(HOME_HEIGHT_BUDGET);
});
