...
import { adminCatalogContent as catalogTexts } from '@/widgets/admin-catalog/content';
...
async function readActive(page: Page, target: Target): Promise<ActiveInfo | null> {
  return page.evaluate((wanted): ActiveInfo | null => {
    ...
    /**
     * Фокус виден: кольцо (обводка или тень) на самом узле **или на его
     * псевдоэлементе** — либо заливка и рамка, которых нет у такого же соседа
     * без фокуса. Второе — про сегменты поля даты: там фокус показан заливкой
     * сегмента, а не кольцом вокруг него, и кольцо между тремя сегментами в
     * одной капсуле читалось бы хуже.
     */
    const focusVisible = (node: HTMLElement, computed: CSSStyleDeclaration): boolean => {
      /* 🔴 Кольцо ищется и на псевдоэлементах узла, а не только на нём самом
         (#870, тот же приём, что в `admin-shell.spec.ts` по #743). Строка
         списка, которая нажимается целиком, рисует кольцо на растянутом
         `::after` своей ссылки (ADR-347): у самого `<a>` при этом стоит
         `outline: none` — рамка вокруг двух слов имени врала бы о размере
         цели, а внешнее кольцо срезала бы область прокрутки таблицы. По стилю
         одного узла фокус выглядит невидимым, хотя на экране обведена вся
         строка. Слепота третья по счёту в проекте: замер тап-зоны (ADR-183),
         два измерителя целей (#548), теперь фокус.
      ...
      if (ringOn(null) || ringOn('::after') || ringOn('::before')) return true;
      ...
      /* Близнец — такой же узел по первому классу: у сегментов даты общий
         ...
         🔴 Псевдоэлементы близнеца намеренно не сравниваются (#870). Ход
         заведён под сегменты поля даты, где фокус показан заливкой самого
         сегмента; на псевдоэлементах сравнение работает иначе и в другую
         сторону — у строк списка `::after` есть у каждой, и любое законное
         различие соседних строк засчиталось бы за фокус. Кольцо на
         псевдоэлементе ловится выше, до близнеца, и второй раз его искать
         незачем: измеритель, слишком охотно отвечающий «видно», — это
         выключенная проверка фокуса, и выглядит она так же зелено. */
