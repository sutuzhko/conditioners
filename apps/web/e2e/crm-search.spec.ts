  ...
  test('🔴 находит дело за пределами показанного периода и не путает его с чужими', async () => {
    ...
    await withAdmin(async (api) => {
      const created = await api.createCrmEvent({
        /* 🔴 Ссылка на справочник, а не ключ перечня (ADR-343). Идентификатор
           известен: стартовую семёрку видов работ заводит миграция, и она же
           задаёт им устойчивые идентификаторы — иначе сценарию пришлось бы
           сначала спрашивать справочник, а раздела для него ещё нет. */
        workTypeId: 'wt_call',
