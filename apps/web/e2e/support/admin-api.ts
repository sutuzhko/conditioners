 ...
 *
 * 🔴 Cookie сессии переносится вручную заголовком, а не кук-хранилищем
 * контекста: сессия админки живёт в Secure-cookie (`server/auth.ts`), а тесты
 * ходят по http. Secure-cookie по http и браузер, и Playwright принимают
 * только на localhost — на любом другом хосте (например `web:3000` из
 * playwright.config) хранилище молча теряло бы сессию, и каждый админ-запрос
 * отвечал бы 401.
 */
 ...
 * а Secure-cookie сессии живёт по http только на localhost (см. выше).
 */
