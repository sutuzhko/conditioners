...
declare global {
  interface Window {
    /** Отмеченные цели. Заводится подменой счётчика и живёт только в тесте. */
    ymGoals?: string[];
  }
}
...
test.describe('Заявка с лендинга', () => {
  ...
  test('форма сохраняет заявку в базу и показывает экран успеха', async ({ page }) => {
    /* 🔴 Цель Метрики проверяется здесь же, а не отдельным сценарием: заявка
       одна, и второй прогон формы стоил бы CI лишней минуты и лишней записи
       в дев-базе (issue #678).

       Счётчик подменён до гидрации и до любого нашего кода: сеть до Метрики
       в тесте не участвует вовсе, проверяется только то, что заявка зовёт
       свою цель. */
    await page.addInitScript(() => {
      window.ymGoals = [];
      window.ymGoal = (goal: string) => {
        window.ymGoals?.push(goal);
      };
    });
    ...
    /* Цель отмечается после ответа сервера, а не по нажатию кнопки: считать
       нужно принятые заявки, а не попытки. */
    expect(await page.evaluate(() => window.ymGoals)).toContain('lead_sent');
  });
});
