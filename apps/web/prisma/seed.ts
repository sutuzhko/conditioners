...
const settings: Record<string, unknown> = {
  ...
  /* `served` — полный список для контактов и разметки, `promise` — короткая
     строка капсулы первого экрана (ADR-126). Срок выезда владелец обещает
     сам: выдумывать его за него нельзя, поэтому здесь только география. */
  area: {
    served: 'Тула и Тульская область',
    promise: 'Тула и Тульская область',
  },
  /* Реквизиты — по составу выбранной формы регистрации (ADR-112, PROJECT §5.1).
     Заглушка стоит только там, где без данных сайт врёт посетителю: ФИО
     предпринимателя, ИНН, ОГРНИП и орган регистрации требует ЗоЗПП ст. 9.

     🔴 Дата регистрации остаётся пустой: «ЗАПОЛНИТЕ В АДМИНКЕ» датой не
     является и схему не проходит. Незаполненной её покажет проверка
     готовности — это честнее, чем заглушка в поле, которое её не принимает.

     Адрес регистрации предпринимателя (как правило домашний, то есть ПДн) и
     банковские реквизиты на витрину не выводятся и обязательными не
     считаются — заглушка в них держала бы настройки «неготовыми» до тех пор,
     пока владелец не впишет то, чего сайту не нужно. */
  legal: {
    form: 'ИП',
    name: TODO,
    inn: TODO,
